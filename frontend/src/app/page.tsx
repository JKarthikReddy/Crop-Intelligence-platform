...
export default function Home() {
  ...
  useEffect(() => {
    fetch("/api/yield")
      .then((res) => res.json())
      .then((data: { forecast: { month: string; value: number }[] }) =>
        setApiYield(data.forecast)
      )
      .catch(() => {
        ...
        >
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">
              Real-time crop intelligence overview
            </p>
          </div>
            ...
            <Card variant="default" className="space-y-2">
              ...
              <div className="flex gap-4">
                {apiYield.map((item) => (
                  <div
                    key={item.month}
                    className="rounded-lg bg-white/5 px-4 py-2 text-center"
                  >
